...
function UICmd:execute(time)
	...
	trigger.action.outTextForGroup(self.asset.groupId, msg, self.displaytime,
		self.displayclear)
	self.asset.cmdpending = false
	return nil
end
...
function ScratchPadSet:_execute(_, _)
	...
	trigger.action.markToGroup(mrkid, title, pos, self.asset.groupId, false,
		"edit me")
...
function CheckPayloadCmd:_execute(_ --[[time]], _ --[[cmdr]])
	local msg = loadout.check(Group.getByName(self.asset.name))
	return msg
end
