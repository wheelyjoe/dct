...
function Asset:unmarshal(data)
	...
	-- need to handle the intel and priority tables special because
	-- even though their keys were numbers when the state was
	-- serialized in json's wisdom it decided to convert them to
	-- strings. So we need to convert back so we can access the data
	-- in our lookups.
