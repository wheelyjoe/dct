...
local AirbaseAsset = class(AssetBase)
...
function AirbaseAsset:_doOneDeparture()
	...
	local flight =
		require("dct.Theater").singleton():getAssetMgr():getAsset(name)
...
function AirbaseAsset:checkDead()
	...
	self:setDead(true)
	--TODO: notify of death, other assets need to know
...
function AirbaseAsset:onDCSEvent(_ --[[event]])
end
...
function AirbaseAsset:generate(assetmgr, region)
	Logger:debug(self.__clsname..":generate called;")
...
local function spawn_despawn(self, action)
	Logger:debug(string.format("%s:spawn_despaawn(%s) called",
		self.__clsname, action))
	local theater = require("dct.Theater").singleton()
	for name, _ in pairs(self.subordinates) do
		...
		if asset then
			...
			Logger:debug(string.format(
				"%s:spawn_despaawn - asset(%s) doesn't exist, "..
				"remove subordinate",
				self.__clsname, name))
...
function AirbaseAsset:spawn(ignore)
	Logger:debug(self.__clsname..":spawn called;")
	...
	--[[
	-- TODO: find another way to monitor airbase as we cannot
	-- reference the theater class since we currently spawn
	-- inside the init function of the Theater class, creating
