...
local StaticAsset = class(BaseAsset)
...
function StaticAsset:_setupDeathGoal(grpdata, static)
	if self._hasDeathGoals then
		...
		self:_addDeathGoal(grpdata.name, BaseAsset.defaultgoal(static))
	end
end
...
function StaticAsset:marshal()
	...
	return utils.mergetables(BaseAsset.marshal(self), tbl)
end
